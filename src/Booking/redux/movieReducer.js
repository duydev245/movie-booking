import { TYPE_COMPLETE, TYPE_DELETED, TYPE_SELECTED } from "../actionType/constant"
...
export const movieReducer = (state = initialState, action) => {
    ...
    switch (action.type) {
        case TYPE_SELECTED: {
            ...
            const isExist = state.bookingList.find((item) => {
                return item.hang === payload.hang && item.soGhe === payload.soGhe
            });

            if (!isExist) {
                // Nếu không có trong bookingList thì mình push nó vào newBookingList 
        ...
        case TYPE_DELETED: {
            const updatedBookingList = state.bookingList.filter(item => item.soGhe !== payload.soGhe);
        ...
        case TYPE_COMPLETE: {
            const bookedSeatNumbers = new Set(state.bookingList.map(ghe => ghe.soGhe));

            const updatedList = state.list.map(item => {
                const updatedDanhSachGhe = item.danhSachGhe.map(ghe =>
                    bookedSeatNumbers.has(ghe.soGhe) ? { ...ghe, "daDat": true } : ghe
                );
                return { ...item, danhSachGhe: updatedDanhSachGhe };
            });
